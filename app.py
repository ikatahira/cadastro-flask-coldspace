...
from models import db, Usuario, Acesso  # ajuste conforme seu projeto
from utils import log  # se tiver função de log

app = Flask(__name__)
app.secret_key = 'sua_chave_secreta_aqui'

# Configuração do banco de dados
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///meubanco.db'  # ou outro caminho
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# Inicializar o banco com o app
db.init_app(app)
...
def dashboard():
    linhas = db.session.query(
        db.func.date(Acesso.data_hora).label('dia'),
        db.func.count(Acesso.id)
    ).group_by('dia').all()
    ...
    return render_template(
        'dashboard.html',
        total=total,
        media_idade=round(float(media_idade), 2),
